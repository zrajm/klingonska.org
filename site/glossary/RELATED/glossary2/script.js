 ...
 * Accept NS5 after adjectival verbs
 * Don't destroy HTML in input (but clear away old color markings)
 * Resizeable input field
    ...
    **                                                                       **
    ...
    function analyze(text, rules) {
        ...
        output('<table class="sortable">' +
            '<thead><tr><th>' +
            '<th><span lang=en>Klingon</span><span lang=sv>Klingonska</span>' +
            '<th><span lang=en>PoS</span><span lang=sv>Typ</span>' +
            '<th><span class=unzap title="Undo word remove.">↶</span>' +
            '<span lang=en>English</span>' +
            '<span lang=sv>Svenska</span></thead>' +
            '<tbody>' +
            Object.keys(glossary).sort(byKlingon).map(function (key) {
                ...
                return tags.map(function (tag) {
                    ...
                    return '<tr class=' + tag + '>' +
                        ...
                        (dict[text] ? (
                            '<span lang=en>' + dict[text][tag].en + '</span>' +
                            '<span lang=sv>' + dict[text][tag].sv + '</span>'
                        ) : '&ndash;') +
                        '</td>' +
                        '</tr>';
                }).join('');
