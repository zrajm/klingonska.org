    ...
    '': {
        ...
        '': {
            ...
            'Method:': {
                ...
                setUp: function () {
                    that = this;
                    this.callback = function (returnValue, thisName, indexName, newName, oldName, cmdName) {
                ...
                'object.sort(sortFunc) => sortedArray': function () {
                    ...
                    obj.push([2, 1]);
                    obj.preChange(this.callback(true, 'preThis', 'preIndex', 'preNew', 'preOld', 'preCmd'));
                ...
                'object.set(newElement) => newElement': function () {
                    var obj = new HookedArray();
                    obj.preChange (this.callback(true, 'preThis', 'preIndex',  'preNew',  'preOld',  'preCmd'));
                    obj.postChange(this.callback(true, 'postThis', 'postIndex', 'postNew', 'postOld', 'postCmd'));
                ...
                'object.push(addArray) => newLength': function () {
                    ...
                    assert.same(this.preThis, obj, 'Pre-callback received bad "this"');
                    assert.equals(this.preIndex,        2, 'Pre-callback received incorrect preIndex');
                ...
                'object.preChange(callback) => HookedArray': {
                    '(no pre-callback)': function () {
                        var obj      = new HookedArray(),
                            postFunc = this.callback(false, 'postThis', 'postIndex', 'postNew', 'postOld', 'postCmd');
                        obj.push(['a']);
                        ...
                        assert.same(this.postThis, obj, 'Post-callback received bad "this"');
                    ...
                    '(pre-callback returning false)': function () {
                        var obj      = new HookedArray(),
                            preFunc  = this.callback(false, 'preThis', 'preIndex',  'preNew',  'preOld',  'preCmd'),
                            postFunc = this.callback(null, 'postThis', 'postIndex', 'postNew', 'postOld', 'postCmd');
                        obj.push(['a']);
                        ...
                        // pre-callback returned false, no postCallback invoked
                        assert.same(  this.postThis,  undefined, 'Post-callback should not have been invoked');
                    ...
                    '(pre-callback returning true)': function () {
                        var obj  = new HookedArray(),
                            preFunc  = this.callback(true, 'preThis', 'preIndex',  'preNew',  'preOld',  'preCmd'),
                            postFunc = this.callback(null, 'postThis', 'postIndex', 'postNew', 'postOld', 'postCmd');
                        obj.push(['a']);
                        assert.same(obj.preChange(preFunc), obj, 'Should return HookedArray object');
                        assert.same(obj.postChange(postFunc), obj, 'Should return HookedArray object');
                ...
                'object.postChange(callback) => HookedArray': {
                    ...
                    '(post-callback returning false)': function () {
                        var obj  = new HookedArray(),
                            postFunc = this.callback(false, 'preThis', 'preIndex', 'preNew', 'preOld', 'preCmd');
                        obj.push(['a']);
                        assert.same(obj.postChange(postFunc), obj, 'Should return HookedArray object');
                    ...
                    '(post-callback returning true)': function () {
                        var obj  = new HookedArray(),
                            postFunc = this.callback(true, 'preThis', 'preIndex', 'preNew', 'preOld', 'preCmd');
                        obj.push(['a']);
                        assert.same(obj.postChange(postFunc), obj, 'Should return HookedArray object');
                        assert.equals(obj.push(['b']),  2, 'Should return array length after successful push');
                        assert.same(this.preThis, obj, 'Pre-callback received bad "this"');
