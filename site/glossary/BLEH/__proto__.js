...
var tableArrayPrototype = {
    init: function (opts) {
        if (Object.prototype.toString.call(opts) !== '[object Object]') {
            throw new TypeError('makeTableArray() argument must option object');
        }
        for (var prop in opts) {               // copy all options into object
            if (opts.hasOwnProperty(prop)) { this[prop] = opts[prop]; }
        }
        this.redraw();                         // redraw HTML table
        return this;
    },
    redraw: function () {
        var that   = this,
...
function makeTableArray(opts) {
    ...
    $('#log').html(tag('pre', JSON.stringify(opts, null, 4)));
    var object = Object.create(tableArrayPrototype).init(opts);
